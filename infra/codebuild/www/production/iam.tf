...
data "aws_iam_policy_document" "iam_pass_role" {
  ...
  statement {
    effect    = "Allow"
    actions   = ["iam:PassRole"]
    resources = ["arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/production-business-dashboard-ecs-execution-role"]
  }
}
...
data "aws_iam_policy_document" "rds" {
  statement {
    ...
    resources = ["arn:aws:rds:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:db:production-business-dashboard-db"]
  }

  statement {
    effect    = "Allow"
    actions   = ["rds-db:connect"]
    resources = ["arn:aws:rds-db:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:dbuser:production-business-dashboard-db/*"]
  }
}
